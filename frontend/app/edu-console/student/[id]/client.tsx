...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      if (reportResult.status === 'fulfilled' && reportResult.value) {
        const reportData = reportResult.value as StudentReport & { error?: boolean };
        if (!reportData.error) {
          setReport(reportData);
        }
      }

      if (competencyResult.status === 'fulfilled' && competencyResult.value) {
        const compData = competencyResult.value as { competencies?: CompetencyItem[] };
        if (compData.competencies) {
          setCompetencies(compData.competencies);
        }
      }

      if (studentListResult.status === 'fulfilled' && studentListResult.value) {
        const listData = studentListResult.value as { students?: Student[] };
        const found = listData.students?.find((s: Student) => s.id === studentId);
        if (found) {
          setStudent(found);
        } else {
          setError('找不到此學員');
  ...
  const handleRequestSuggestions = async () => {
    ...
    try {
      const result = await adminService.getAiSuggestions(studentId) as { suggestions?: AiSuggestion[] };
