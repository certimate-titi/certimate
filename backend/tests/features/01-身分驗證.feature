Feature: 身分驗證
  ...
  Background:
    Given 系統中有以下使用者帳號：
      ...
      | 2        | bob@example.com      | email    | PRO         | USER  | 已啟用 |
      | 3        | carol@example.com    | google   | ULTRA       | USER  | 已啟用 |
  ...
  Rule: 後置（回應）- 登入後導覽列依 role 與 subscription_tier 顯示對應功能入口
    ...
    # DB: users.subscription_tier = 'ULTRA' → 顯示「教育管理」連結 (/edu-console)
    # DB: users.role = 'ADMIN' → 顯示「平台管理」連結 (/super-admin/dashboard)

    Example: ULTRA 用戶登入後導覽列顯示「教育管理」入口
      Given 使用者 "carol@example.com" 訂閱方案為 "ULTRA"
      When 使用者 "carol@example.com" 成功登入系統
      Then 登入後的回應應包含 "subscription_tier": "ULTRA"
    ...
    Example: FREE / PRO 一般用戶登入後導覽列不顯示管理入口
  ...
  Rule: 後置（狀態）- 登入頁支援 Firebase Google 一鍵登入流程

    # 設計備註：前端使用 Firebase signInWithPopup 取得 Google ID token，
    # 再呼叫後端 /auth/google-sso 交換 JWT（非傳統 OAuth redirect flow）。

    Example: 使用者點擊 Google 登入按鈕後開啟 Firebase popup
      When 使用者在登入頁面點擊「以 Google 帳號登入」按鈕
      Then 前端應開啟 Firebase Google 登入 popup

    Example: Firebase Google 登入授權成功後系統自動完成登入並導向儀表板
      Given 使用者 "carol@example.com" 已有 Google SSO 帳號且狀態為 "已啟用"
      When 使用者完成 Google 登入授權且 Email 為 "carol@example.com"
      Then 操作成功
      And 回應應包含有效的 JWT 存取憑證
