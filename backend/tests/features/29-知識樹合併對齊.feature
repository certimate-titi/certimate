...
Feature: 知識樹合併對齊
  ...
  Rule: 後置（映射）- 統一萃取後 resource_chunks 必須映射到新的統一節點

    Example: 上傳資源後統一萃取 — chunks 應映射到統一知識節點
      Given 使用者 "pro@example.com" 已上傳資源 "信託教材.pdf"（科目 ID: 1）且有 5 個分塊
      And 考科 "信託業業務人員" 已有統一知識樹：
        | 節點名稱     | 層級 | 來源        |
        | 信託法規     | 1    | ai_unified |
        | 信託契約     | 2    | ai_unified |
        | 受託人義務   | 2    | ai_unified |
  ...
  Rule: 不變量（結構）- 統一知識樹的 depth-1 節點數量不得超過 6 個
    ...
    設計原則：
      ...
      若素材涵蓋超過 6 個面向，AI 應合併語意相近的面向。
      細節知識點應歸入第二層（節）或第三層（小節），不得提升為核心主題。

    Example: 統一萃取結果的核心主題不超過 6 個
      Given 考科 "信託業業務人員" 已匯入 180 題考古題
      And 使用者 "pro@example.com" 已上傳 3 份教材
      When 系統執行統一知識樹萃取
      Then 操作成功
      And 統一知識樹的 depth=1 節點數量應 <= 6
      And 每個 depth=1 節點下應有至少 2 個 depth=2 子節點

    Example: 大量素材仍維持六主題上限
      Given 考科 "信託業業務人員" 已有 5 個核心主題
      And 使用者 "pro@example.com" 又上傳了 2 份新教材，涵蓋 3 個全新面向
      When 系統執行統一知識樹合併對齊
