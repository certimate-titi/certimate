...
**便條紙格式**：`CiC(<CATEGORY>): <內容>`，CATEGORY 為 `AMB`/`ASM`/`GAP`/`CON`/`BDY`，每個 .activity 獨立使用。
...
| 分類 | 代碼 | 何時標記 |
|------|------|---------|
| Ambiguity | `AMB` | 需求存在多種合理解讀（含分支條件可能不完整） |
| Assumption | `ASM` | AI 做了一個未經確認的選擇 |
| Gap | `GAP` | 資訊完全缺失，無法確定該步驟的詳細行為 |
| Conflict | `CON` | 同一流程中前後文衝突 |
| Boundary | `BDY` | 範圍邊界不明確（含存在其他可行路徑但暫時未建模） |
...
Feature: <功能名>
  ...
  Rule: 前置（狀態）- <推斷的約束>  # CiC(ASM): 錯誤訊息為「...」？
  ...
  Rule: 後置（狀態）- <推斷的後置效果>  # CiC(GAP): 後置狀態為何？
...
1. **解決便條紙**：定位對應便條紙，刪除 `CiC(<CATEGORY>): ...` 標記
