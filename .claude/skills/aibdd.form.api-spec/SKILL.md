...
**所有便條紙一律使用 CiC 格式，包在 YAML 原生行尾 comment 內：**
...
paths:
  /orders:
    post: # CiC(AMB): 不確定是 POST 還是 PUT，取決於後端實作
```
...
在以下情況於 api.yml 對應位置加上便條紙（YAML 行尾 `# CiC(<CATEGORY>): 內容`）：

| 代碼 | 何時標記 |
|------|---------|
| `GAP` | 無法從 .feature 推斷 HTTP Method 或 Path 時 |
| `ASM` | 推斷了 enum 值域但不確定是否完整時 |
| `AMB` | 同一操作可能有多種 HTTP Method（如 PUT vs PATCH）時 |
