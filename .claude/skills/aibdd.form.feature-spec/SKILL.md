...
**所有便條紙、佔位符、待澄清標記，無論出現在 `.feature` 或 `.activity` 任何位置，一律使用 CiC 格式：**
...
理由：系統使用 Grep 工具搜尋 `CiC\(` 統一掃描所有視圖的未解便條紙，非此格式的標記將被忽略、導致遺漏。
...
Rule: 前置（狀態）- 商品庫存必須大於 0
  # CiC(GAP): 庫存為 0 時的錯誤訊息為何？

  Example: 庫存不足時操作失敗
    # CiC(ASM): 假設單次最多購買 10 件
```
