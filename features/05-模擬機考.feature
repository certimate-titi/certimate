Feature: 模擬機考
  ...
  Rule: 後置（狀態）- 使用者選擇答案後應自動儲存至 LocalStorage 與後端
    ...
    Example: 選擇答案後作答記錄被儲存
      ...
      And 瀏覽器 LocalStorage 中應更新對應的作答記錄

  Rule: 後置（狀態）- 標記複查後題目應在題號導覽網格顯示旗幟圖示
  ...
  Rule: 後置（狀態）- 計時結束後測驗自動提交並更新狀態為 SUBMITTED

    Example: 倒數時間歸零時系統自動提交測驗
      Given 使用者 "alice@example.com" 已開始測驗 1，剩餘時間為 1 秒
      When 系統倒數時間歸零
      Then 測驗 1 應自動提交
