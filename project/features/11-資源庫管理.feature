Feature: 資源庫管理與連鎖清除防呆機制
  ...
  # ========== 非同步上傳處理（Cloud Tasks Pipeline）==========

  Rule: 後置（非同步）- 資源上傳完成後立即回 202，背景處理走 Worker

    Example: 上傳檔案後主服務立即回應 202 Accepted，不等待解析完成
      When 使用者 "alice@example.com" 上傳資源檔案並送出
      Then HTTP 狀態碼應為 202
      And 回應中應包含 resource_id

    Example: Worker 處理端點驗收 — Cloud Tasks 推送 payload 正確觸發 pipeline
      When Cloud Tasks 推送 process-resource payload 給 Worker
      Then HTTP 狀態碼應為 200
      And 回應中 ok 應為 true
