...
Feature: 階層式難度遞進
  ...
  # ========== 回溯觸發條件 ==========
  # 回溯觸發為 OR 關係：條件 A（連續答錯 2 題）或條件 B（節點答對率 < 40%），其一即觸發。
  # 兩條件同時滿足時依先觸發者處理，不重複回溯。回溯後重置連續錯題計數器。

  Rule: 前置（條件）- 回溯觸發條件為 OR 關係，任一條件滿足即觸發

    Example: 條件A — 連續答錯 2 題觸發回溯（快速信號）
      Given 系統的回溯觸發條件為「連續答錯 2 題 OR 節點答對率 < 40%」
      And 使用者 "pro@example.com" 在節點 "忠實義務" 連續答錯 2 題
      When 系統計算下一題的出題策略
      Then 系統應觸發回溯到父節點 "受託人義務"
      And 連續錯題計數器應歸零

    Example: 條件A 未達門檻 — 不觸發回溯
      Given 系統的回溯觸發條件為「連續答錯 2 題 OR 節點答對率 < 40%」
      And 使用者 "pro@example.com" 在節點 "忠實義務" 答對 1 題後答錯 1 題
      And 使用者 "pro@example.com" 在節點 "忠實義務" 的答對率為 60%
      When 系統計算下一題的出題策略
      Then 系統不應觸發回溯（兩條件皆未滿足）
      And 下一題仍在節點 "忠實義務" 出題

    Example: 條件B — 節點答對率低於 40% 觸發回溯（累積信號）
      Given 系統的回溯觸發條件為「連續答錯 2 題 OR 節點答對率 < 40%」
      And 使用者 "pro@example.com" 在節點 "忠實義務" 的答對率為 35%（答對 7 / 總 20）
      And 使用者 "pro@example.com" 最近一題答對（非連續錯誤）
      When 系統計算下一題的出題策略
      Then 系統應觸發回溯到父節點 "受託人義務"

    Example: 兩條件同時滿足時不重複回溯
      Given 系統的回溯觸發條件為「連續答錯 2 題 OR 節點答對率 < 40%」
      And 使用者 "pro@example.com" 在節點 "忠實義務" 連續答錯 2 題
      And 使用者 "pro@example.com" 在節點 "忠實義務" 的答對率為 30%
      When 系統計算下一題的出題策略
      Then 系統應觸發回溯到父節點 "受託人義務"（僅回溯一次，不重複）
