...
Feature: 訂閱管理與多階層控制
  ...
  Rule: 後置（回應）- 查看訂閱資訊應回傳當前方案、下次扣款日與額度比較
    ...
    Example: FREE 用戶查看訂閱資訊取得升級比較
      ...
      And 回應應包含：
        ...
        | next_billing  | null |
      And 回應應包含各方案額度比較：
        | 方案          | AI 對話次數 | AI 教練月配額 | 上傳數 | 考試數 | 每次測驗題數上限 | Vision OCR |
        | FREE          | 3           | 0（追問3次/節點）| 5    | 10     | 10              | 0          |
        | PRO_199       | 30          | 20（基礎教練） | 50     | 100    | 50              | 0          |
        | PRO_PLUS_399  | 200         | 100（完整教練）| 200    | 500    | 100             | 50         |
        | ULTRA_1599    | 無限        | 無限（進階教練）| 無限   | 無限   | 無限            | 500        |
  ...
  Rule: 後置（回應）- EDU 方案應出現在方案權限對照表中
    ...
    Example: 查看訂閱資訊時 EDU 方案列於比較表中
      ...
      Then 操作成功
      And 回應應包含各方案額度比較：
        | 方案          | AI 對話次數 | AI 教練月配額 | 上傳數 | 考試數 | 每次測驗題數上限 | Vision OCR | 自主出題 |
        | FREE          | 3           | 0（追問3次/節點）| 5    | 10     | 10              | 0          | 是       |
        | PRO_199       | 30          | 20（基礎教練） | 50     | 100    | 50              | 0          | 是       |
        | PRO_PLUS_399  | 200         | 100（完整教練）| 200    | 500    | 100             | 50         | 是       |
        | ULTRA_1599    | 無限        | 無限（進階教練）| 無限   | 無限   | 無限            | 500        | 是       |
        | EDU           | 5           | 5（基礎，共用機構配額池）| 0 | 無限 | 50             | 0          | 否       |
