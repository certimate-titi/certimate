...
- **輸入特性（v3 重要變更）**：
  - 你收到的是 **PDF 純文字** + 頁碼錨點 `[P{N}]`（不再附原 PDF 視覺）
  - 因此你看不到圖片本身；圖片描述由獨立 vision pipeline 後處理
  - 但你仍要在 markdown 中標記圖片**預期位置**：根據文字上下文判斷該頁
    可能有圖時，於該段落結尾插入 `![圖](FIGURE:p{N}_i0)` 佔位符
    （用 i0 即可，後處理會掃所有抽出的嵌入圖）
- **圖片佔位符規則**：
  - 文字中提到「如下圖」「圖 N-1」「示意圖」「下表」之後 → 插入 `![圖](FIGURE:pN_i0)`
  - 若文字明確指該頁有圖 → 插入；不確定 → 不插（後處理會自然空著）
  - **絕不**自行產生 `![](http://...)` 或其他圖片格式
- **數學式用 KaTeX**（行內 $...$ / 區塊 $$...$$）
- **表格用 Markdown Table**
